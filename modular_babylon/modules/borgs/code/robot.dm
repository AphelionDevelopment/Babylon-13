...
/mob/living/silicon/robot/on_lying_down(new_lying_angle)
	...
	if(model && model.model_features && (TRAIT_R_TALL in model.model_features))
		...
		if(!tabled)
			...
			playsound(src, 'modular_babylon/master_files/sound/effects/robot_sit.ogg', 25, TRUE)
			return
		else
			new /obj/effect/temp_visual/mook_dust/robot/table(get_turf(src))
			playsound(src, 'modular_babylon/master_files/sound/effects/robot_bump.ogg', 50, TRUE)
 ...
 *
 * model_features is defined in modular_babylon\modules\altborgs\code\modules\mob\living\silicon\robot\robot_model.dm.
 */
