...
/obj/item/hairbrush
	...
	icon = 'modular_babylon/modules/hairbrush/icons/hairbrush.dmi'
	icon_state = "brush"
	inhand_icon_state = "inhand"
	lefthand_file = 'modular_babylon/modules/hairbrush/icons/inhand_left.dmi'
	righthand_file = 'modular_babylon/modules/hairbrush/icons/inhand_right.dmi'
...
/obj/item/hairbrush/proc/brush(mob/living/target, mob/living/user)
	if(ishuman(target))
		...
		if(user.combat_mode && human_target != user)
			...
			playsound(human_target, 'modular_babylon/modules/hairbrush/sounds/rough_brush.ogg', 30, extrarange = -6, ignore_walls = FALSE)
			return
		...
		if(human_target == user)
			...
			if(HAS_TRAIT(user, TRAIT_HAIR_EXPERT)) // Do they have the hair expert trait? If so, give them the better moodlet.
				...
				human_target.add_mood_event("brushed", /datum/mood_event/brushed, user, brush_target)
		playsound(human_target, 'modular_babylon/modules/hairbrush/sounds/brush.ogg', 30, extrarange = -6, ignore_walls = FALSE)
		...
		playsound(target, 'modular_babylon/modules/hairbrush/sounds/brush.ogg', 30, extrarange = -6, ignore_walls = FALSE)
