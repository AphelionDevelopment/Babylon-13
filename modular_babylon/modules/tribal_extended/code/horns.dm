...
/obj/item/blowing_horn
	...
	icon = 'modular_babylon/modules/tribal_extended/icons/items_and_weapons.dmi'
...
/obj/item/blowing_horn/attack_self(mob/living/user)
	...
	if(!COOLDOWN_FINISHED(src, bhorn_cooldown))
		...
		for (var/mob/hearing_player as anything in SSmobs.clients_by_zlevel[user.z])
			...
			hearing_player.playsound_local(bhorn_origin, 'modular_babylon/master_files/sound/items/blow_horn.ogg', 150, TRUE)
...
/obj/structure/war_horn/attack_hand(mob/living/user)
	...
	if (!is_mining_level(user.z))
		...
		playsound(location, 'modular_babylon/modules/admin/sound/duckhonk.ogg', 100, TRUE)
		return
	...
	for (var/mob/hearing_player in GLOB.player_list)
		...
		hearing_player.playsound_local(location, 'modular_babylon/master_files/sound/items/war_horn.ogg', 150, TRUE)
