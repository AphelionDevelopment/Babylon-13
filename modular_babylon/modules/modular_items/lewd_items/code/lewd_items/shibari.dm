...
/obj/item/stack/shibari_rope/attack_self(mob/user)
	switch(tightness)
		if(SHIBARI_TIGHTNESS_HIGH)
			...
			playsound_if_pref(loc, 'modular_babylon/modules/modular_items/lewd_items/sounds/latex.ogg', 25)
			balloon_alert(user, "slightly tightened the ropes")
		if(SHIBARI_TIGHTNESS_LOW)
			tightness = SHIBARI_TIGHTNESS_MED
			playsound_if_pref(loc, 'modular_babylon/modules/modular_items/lewd_items/sounds/latex.ogg', 50)
			balloon_alert(user, "moderately tightened the ropes")
		if(SHIBARI_TIGHTNESS_MED)
			tightness = SHIBARI_TIGHTNESS_HIGH
			playsound_if_pref(loc, 'modular_babylon/modules/modular_items/lewd_items/sounds/latex.ogg', 75)
