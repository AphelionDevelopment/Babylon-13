/obj/effect/overlay/gunpoint_effect
	icon = 'modular_babylon/modules/gunpoint/icons/targeted.dmi'
	icon_state = "locking"
	layer = FLY_LAYER
	plane = GAME_PLANE
...
/datum/gunpoint/proc/lock_on()
	if(src) //if we're not present then locking on failed and this datum is deleted
		...
		playsound(get_turf(source), 'modular_babylon/modules/gunpoint/sound/targeton.ogg', 50,1)
...
/datum/gunpoint/proc/click_destroy()
	SIGNAL_HANDLER
	if(locked)
		playsound(get_turf(source), 'modular_babylon/modules/gunpoint/sound/targetoff.ogg', 50,1)
	qdel(src)
