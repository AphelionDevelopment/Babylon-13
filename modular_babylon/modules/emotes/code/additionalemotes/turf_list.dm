...
/obj/structure/mark_turf
	name = "turf"
	icon = 'modular_babylon/master_files/icons/effects/turf_effects.dmi'
...
/obj/structure/mark_turf/Initialize(mapload, current_turf)
	...
	switch(current_turf)
		if("web")
			...
			playsound(get_turf(src), 'modular_babylon/master_files/sound/effects/weave.ogg', 25, TRUE)
		...
		if("water")
			...
			icon_state = "water"
			src.add_overlay(image('modular_babylon/master_files/icons/effects/turf_effects.dmi', "water_top", EXTRA_ABOVE_MOB_LAYER))
			flick_overlay_static(image('modular_babylon/modules/liquids/icons/obj/effects/splash.dmi', "splash", EXTRA_ABOVE_MOB_LAYER), 20)
			playsound(get_turf(src), 'modular_babylon/master_files/sound/effects/watersplash.ogg', 25, TRUE)

		if("smoke")
			name = "blazing mist"
			desc = "It's a storm of smoke."
			icon_state = "smoke"
			src.add_overlay(image('modular_babylon/master_files/icons/effects/turf_effects.dmi', "smoke_top", EXTRA_ABOVE_MOB_LAYER))
		...
		if("holoseat")
			...
			icon_state = "holoseat"
			src.add_overlay(image('modular_babylon/master_files/icons/effects/turf_effects.dmi', "holoseat_top", EXTRA_ABOVE_MOB_LAYER))
			playsound(get_turf(src), 'sound/machines/compiler/compiler-stage2.ogg', 25, TRUE)
		...
		if("slime")
			...
			switch(rand(1,1000))
				if(-INFINITY to 400)
					...
					src.add_overlay(image('modular_babylon/master_files/icons/effects/turf_effects.dmi', "slimeobj1_top", EXTRA_ABOVE_MOB_LAYER))
				if(400 to 800)
					icon_state = "slimeobj2"
					src.add_overlay(image('modular_babylon/master_files/icons/effects/turf_effects.dmi', "slimeobj2_top", EXTRA_ABOVE_MOB_LAYER))
				if(800 to 980)
					icon_state = "slimeobj3"
					src.add_overlay(image('modular_babylon/master_files/icons/effects/turf_effects.dmi', "slimeobj3_top", EXTRA_ABOVE_MOB_LAYER))
				if(980 to INFINITY)
		...
		if("dust")
			...
			icon = 'modular_babylon/master_files/icons/effects/turf_effects_64.dmi'
			icon_state = "dust"
			pixel_x = -16
			src.add_overlay(image('modular_babylon/master_files/icons/effects/turf_effects_64.dmi', "dust_top", EXTRA_ABOVE_MOB_LAYER))
			playsound(get_turf(src), 'modular_babylon/master_files/sound/effects/wing_flap.ogg', 25, TRUE)
		...
		if("tails")
			...
			var/mutable_appearance/overlay = mutable_appearance('modular_babylon/master_files/icons/effects/turf_effects_64.dmi', "tails_top", EXTRA_ABOVE_MOB_LAYER, src)
		...
		if("coil")
			...
			icon_state = "naga"
			pixel_x = -16
			var/mutable_appearance/overlay = mutable_appearance('modular_babylon/master_files/icons/effects/turf_effects_64.dmi', "naga_top", EXTRA_ABOVE_MOB_LAYER, src)
			overlay.appearance_flags = TILE_BOUND|PIXEL_SCALE|KEEP_TOGETHER
			src.add_overlay(overlay)
			playsound(get_turf(src), 'modular_babylon/modules/emotes/sound/emotes/hiss.ogg', 25, TRUE)
