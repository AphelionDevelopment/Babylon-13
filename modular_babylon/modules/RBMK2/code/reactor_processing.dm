/obj/machinery/power/rbmk2/process(seconds_per_tick)
	...
	if(active && rod_mix.temperature > stored_rod.temperature_limit || last_power_generation > max_power_generation*(1.1 + rand()) )
		...
		var/chosen_sound = pick(
			'modular_babylon/modules/RBMK2/sounds/failure01.ogg',
			'modular_babylon/modules/RBMK2/sounds/failure02.ogg',
			'modular_babylon/modules/RBMK2/sounds/failure03.ogg',
			'modular_babylon/modules/RBMK2/sounds/failure04.ogg',
		)
...
/obj/machinery/power/rbmk2/process_atmos() //Only turf air related stuff is handled here.
	...
	if(stored_rod && meltdown)
		...
		playsound(src, 'modular_babylon/modules/RBMK2/sounds/ionization.ogg', 50, TRUE, extrarange = ionize_air_range)
	else
