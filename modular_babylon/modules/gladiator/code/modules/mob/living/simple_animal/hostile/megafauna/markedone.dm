...
/mob/living/simple_animal/hostile/megafauna/gladiator
	...
	icon = 'modular_babylon/modules/gladiator/icons/markedone.dmi'
	icon_state = "marked1"
	icon_dead = "marked_dying"
	attack_verb_simple = "cleave"
	attack_verb_continuous = "cleaves"
	attack_sound = 'modular_babylon/master_files/sound/weapons/bloodyslice.ogg'
	...
	for(var/turf/targeted as anything in spinningturfs)
		...
		for(var/mob/living/slapped in targeted)
			if(!FAST_FACTION_CHECK(faction, slapped.faction, null, null, FALSE) && !(slapped in hit_things))
				playsound(src, 'modular_babylon/modules/gladiator/Clang_cut.ogg', 75, 0)
