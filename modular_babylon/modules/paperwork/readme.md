...
The module hooks into the photocopier's GLOBAL Init process to seamlessly inject custom Nova templates from `modular_babylon/modules/paperwork/config/blanks.json` into the global `paper_blanks` list on server boot.
...
- **Added (Module Code):** `modular_babylon/modules/paperwork/photocopier.dm` - Contains the `init_paper_blanks_nova()` global proc definition and its JSON merging logic.
- **Added (Module Config):** `modular_babylon/modules/paperwork/config/blanks.json` - The dedicated modular JSON file for defining Nova-specific photocopier templates.
