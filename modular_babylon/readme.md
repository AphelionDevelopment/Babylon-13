...
And then you'll want to establish your core folder that you'll be working out of which is normally your module ID. E.g. `modular_babylon/modules/shuttle_toggle`
...
- **_Example:_** You're adding a new lavaland mob.
  ...
  First of all you create your modular folder. E.g. `modular_babylon/modules/lavalandmob`
  ...
  ```byond
    /mob/lavaland/newmob
      icon = 'modular_babylon/modules/lavalandmob/icons/mob.dmi'
      icon_state = "dead_1"
      sound = 'modular_babylon/modules/lavalandmob/sounds/boom.ogg'
  ```
...
Example: `code/modules/mob/living/living.dm` -> `modular_babylon/master_files/code/modules/mob/living/living.dm`
...
In short, most of the modular code will be placed in the subfolders of your main module folder **`modular_babylon/modules/yourmodule/code/`**, with similar rules as with the assets. Do not mirror core code folder structures inside your modular folder.

For example, `modular_babylon/modules/xenoarcheaology/code` containing all the code, tools, items and machinery related to it.
...
- **Removal:**
  ...
  *Please avoid this however, only to be done if there is no other option.
  ...
  ```byond
  /* // NOVA EDIT REMOVAL START - SHUTTLE_TOGGLE - (Moved to modular_babylon/shuttle_toggle/randomverbs.dm)
...
That folder is **`code/__DEFINES/~babylon_defines`**, in which you can add them to the existing files, or create those files as necessary.
...
- Good: /modular_babylon/modules/example_module/code/disease_mob.dm
- Bad: /modular_babylon/modules/example_module/code/modules/antagonists/disease/disease_mob.dm

**Icons**: Any .DMI files must go in here.

- Good: /modular_babylon/modules/example_module/icons/mining_righthand.dmi
- Bad: /modular_babylon/modules/example_module/icons/mob/inhands/equipment/mining_righthand.dmi
