...
SUBSYSTEM_DEF(player_ranks)
	...
	/// The Star player rank controller.
	var/datum/player_rank_controller/star/star_controller
...
/**
 * Returns whether or not the user is qualified as a Star.
 *
 * Arguments:
...
/// based on configs.
/datum/controller/subsystem/player_ranks/proc/load_stars()
	PROTECTED_PROC(TRUE)
	...
	if(!SSdbcore.Connect())
		var/message = "Failed to connect to database in load_stars(). Reverting to legacy system."
		log_config(message)
		log_game(message)
		message_admins(message)
