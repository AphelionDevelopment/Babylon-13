...
/datum/controller/subsystem/job/proc/check_job_eligibility(mob/dead/new_player/player, datum/job/possible_job, debug_prefix = "", add_job_to_log = FALSE)
	...
	if(GLOB.star_restrictions && possible_job.stars_only && !SSplayer_ranks.is_star(player.client))
		job_debug("[debug_prefix] Error: [get_job_unavailable_error_message(JOB_NOT_STAR)], Player: [player][add_job_to_log ? ", Job: [possible_job]" : ""]")
		return JOB_NOT_STAR
