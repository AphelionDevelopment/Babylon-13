...
/datum/controller/subsystem/job/proc/check_job_eligibility(mob/dead/new_player/player, datum/job/possible_job, debug_prefix = "", add_job_to_log = FALSE)
	...
	if(GLOB.babylon_star_restrictions && possible_job.babylon_stars_only && !SSplayer_ranks.is_babylon_star(player.client))
		job_debug("[debug_prefix] Error: [get_job_unavailable_error_message(JOB_NOT_BABYLON_STAR)], Player: [player][add_job_to_log ? ", Job: [possible_job]" : ""]")
