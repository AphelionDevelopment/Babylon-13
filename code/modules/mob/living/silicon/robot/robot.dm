...
/mob/living/silicon/robot/proc/pick_model()
	...
	if(!length(GLOB.cyborg_base_models_icon_list))
		for(var/option in GLOB.cyborg_model_list)
			...
			GLOB.cyborg_base_models_icon_list[option] = image(icon = 'modular_babylon/master_files/icons/mob/robots.dmi', icon_state = model_icon) // NOVA EDIT - CARGO BORGS - ORIGINAL: model_icons[option] = image(icon = 'icons/mob/robots.dmi', icon_state = model_icon)
	// NOVA EDIT END
