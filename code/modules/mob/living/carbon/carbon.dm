...
<<<<<<< HEAD
	if(HAS_TRAIT(src, TRAIT_NIGHT_VISION) && length(GET_TRAIT_SOURCES(src, TRAIT_NIGHT_VISION) - QUIRK_TRAIT)) // BABYLON_EDIT: the night vision quirk keeps its tuned color cutoffs only, no flat floor (see _eyes.dm NOVA EDIT)
=======
