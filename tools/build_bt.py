...
# modular_babylon/ trees share one flat build/behavior_trees/ tree. Mirrors BT_COMPILED_PATH
# in code/__DEFINES/ai/behavior_trees.dm exactly
_KNOWN_ROOTS = ('code/', 'modular_babylon/')


def strip_known_root(rel_to_repo: str) -> str:
    """Strip a leading code/ or modular_babylon/ from a repo-relative path, if present."""
...
def parse_defines(repo_root: Path) -> dict:
    ...
    dm_files = sorted(f for d in (repo_root / 'code', repo_root / 'modular_babylon') if d.is_dir() for f in d.rglob('*.dm')) # NOVA EDIT CHANGE - ORIGINAL:  defines_dir = repo_root / 'code'
...
def main() -> int:
    ...
    bt_files = sorted(repo_root.glob('code/**/*.bt.json')) + sorted(repo_root.glob('modular_babylon/**/*.bt.json')) # NOVA EDIT CHANGE - ORIGINAL: bt_files = sorted(repo_root.glob('code/**/*.bt.json'))
    ...
    for src_path in bt_files:
        # The compiled file mirrors the source path relative to its root (code/ or modular_babylon/), so trees that share a basename dont fucking break. # NOVA EDIT CHANGE - ORIGINAL: # The compiled file mirrors the source path relative to code/, so trees that share a basename dont fucking break.
